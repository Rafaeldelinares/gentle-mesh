...
- [x] Task 21: Implementar `TerritoryManager` y endpoints de peering federado M2M (`/v1/mesh/peers`, `/v1/mesh/territory`) con TDD y detección unificada de colisiones territoriales locales y federadas (`pkg/server/federation`, `pkg/server/http`). (Commit pendiente, todos los tests pasando con `-race`)
